        ...
        if(this.state.stocks!==null && this.state.stocks!=="No stocks added" && this.state.stocks!==undefined ) {
           ...
           var user_watchlist = this.state.stocks;
            ...
            this.ws.onmessage = evt => {
                console.log("stocks inside DIDMOUNT: ",this.state.stocks)
            ...
            if(message.type==="trade"){ 
                ...
                }).then(()=>this.setstate({stocks:user_watchlist},()=> this.updatePage))
                ;
                // this.props.history.push("/profile",[[{email:this.state.email}],user_watchlist])
                }
            console.log(message)
            }
        }
...
updatePage(){
    ...
    this.state.stocks.forEach(symbol=>{
        this.ws.send(JSON.stringify({'type':'subscribe', 'symbol': symbol.Symbol}));
        console.log("Inside UpdatePage: Subscribed To: ", symbol.Symbol)
    })
