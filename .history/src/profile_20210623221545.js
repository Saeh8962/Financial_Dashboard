...
class Profile extends Component { 
    ...
    getStockInfo(){
        ...
        .then(api_result=>{
            this.setState({stocks:{stock_info:api_result}} )
            return this.state;
        }) 
        .catch((error)=>{
...
render(){
    ...
    <Dropdown style = {{marginBottom:"10px"}} isOpen={this.state.dropdownOpen}>
		...
		<div className = "ArtistLine" style = {{marginBottom:"25px"}}>
			<button id = {this.state.stocks.symbol} onClick = {(e)=>{this.doNothing(e)}} className = "artistButton">{this.state.stocks.symbol} ${this.state.stocks.current_price}</button>
