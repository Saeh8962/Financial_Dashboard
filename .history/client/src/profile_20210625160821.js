...
class Profile extends Component { 
    constructor(props){
        ...
        this.addUserStock = this.addUserStock(this);
        this.updatePage= this.updatePage(this);
...
getUserStocks(){
    ...
    .then(query_result => this.setState({stocks:query_result},()=> this.updatePage()))
    .catch((error)=>{
        return Promise.reject(error);
        });
}
...
render(){
    ...
    <Dropdown style = {{marginBottom:"10px"}} isOpen={this.state.dropdownOpen}>
		...
		<div className = "ArtistLine" style = {{marginBottom:"25px"}}>
			...
			<button onClick = {(e)=>{this.doNothing(e)}} className = "playArtistButton">{"\u25B6"}</button>
			<button onClick = {(e)=>{this.doNothing(e)}} className = "removeButton">X</button> 
		</div>
            )}
          </div>
        </DropdownMenu>
