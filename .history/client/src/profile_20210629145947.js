...
class Profile extends Component { 
    constructor(props){
        ...
        this.getLiveTrades= this.getLiveTrades.bind(this);
...
componentDidMount(){
    ...
    var user_watchlist = this.state.stocks
    this.ws.onopen = () => {
    ...
    this.ws.onmessage = evt => {
        ...
        if(message.type==="trade"){ 
            Object.values(user_watchlist.symbol).forEach(symbol=>{
                var tradePrice= data.filter((i,n)=>i.s===symbol.Symbol);
...
componentWillUnmount(){
    Object.values(this.state.stocks.Symbol).forEach(symbol=>this.ws.send(JSON.stringify({'type':'unsubscribe', 'symbol': this.state.stocks.Symbol})))
    this.ws.close();
