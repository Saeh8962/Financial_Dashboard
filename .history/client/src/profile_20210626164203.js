...
class Profile extends Component { 
    constructor(props){
        ...
        // this.UpdateRemoveStockOnPage=this.UpdateRemoveStockOnPage.bind(this);
...
UpdateStockOnPage(){
    ...
    if(this.state.Remove_Status === 'error: Failed to remove stock'){
        alert("There as been an error please log back in.");
    }
    else{
...
removeStock(stock){
    ...
    .then((res)=>{
      ...
      .then((result)=> this.setState({remove_status:result},()=>this.UpdateStockOnPage()));
...
render(){
    ...
    <Dropdown style = {{marginBottom:"10px"}} isOpen={this.state.dropdownOpen}>
		...
		<div className = "ArtistLine" style = {{marginBottom:"25px"}}>
			...
			<button onClick = {this.removeStock(stocks.Symbol)} className = "removeButton">X</button> 
		</div>
            )}
          </div>
        </DropdownMenu>
