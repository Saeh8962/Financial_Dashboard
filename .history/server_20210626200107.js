...
app.post('/api/removeUserStock',function(req, res){
    ...
    db.query(sql_delete_query,[email,stock_to_remove],function(err,result){
        ...
        if(result.length ===0){
            ...
            var sql_check_delete ='select symbol from stock_app.user_stocks where Uemail = ? and symbol = ?';
...
app.post('/api/getUserStocks',function(req,res){
    ...
    var sql_query = "select Symbol,currentPrice from stock_app.stocks where Symbol in (Select symbol from stock_app.user_stocks where Uemail = ?)"
